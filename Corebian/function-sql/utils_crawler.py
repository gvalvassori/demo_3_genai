...
def generate_reference(result, n_results):
    ...
    if ENGINE_TYPE == "unstructured":
        print("procesanto engine unstructured")
    ...
    elif  ENGINE_TYPE == "structured":
        print("Procesando la query ",result)
        query = result
        df = (client2.query(query.replace("GROUP_CONCAT", "STRING_AGG")).result().to_dataframe())
        df_string = df.to_string()
        return {"link": query, "long_snippet": df_string, "title": "catalogo"}
